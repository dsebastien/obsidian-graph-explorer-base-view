...
- The `graph-explorer:settings-changed` event fires inside the serialized
  `updateSettings`, strictly AFTER the write lands — views must never re-read
  state that a failed write rolled back.
- The stylesheet is @layer'd, so the `.ge-settings-embed` block rule lives
  OUTSIDE the layers (unlayered CSS beats layered CSS regardless of
  specificity).
- Property-name text controls fall back to their defaults when cleared, in
  `setControlValue` — the previous tab's `value || 'default'` behavior.
